public class SearchMatrix {
  ...
  private boolean searchFrom(int[][] matrix, int target, int row, int col) {
    ...
    if (matrix[row][col] < target) {
      return searchFrom(matrix, target, row + 1, col);
    } else {
      return searchFrom(matrix, target, row, col - 1);
    }
  }
  ...
  public boolean searchMatrix(int[][] matrix, int target) {
    ...
    return searchFrom(matrix, target, 0, matrix[0].length - 1);
  }
