...
public class HumanPyramid {
   ...
   * row along the ground, and each subsequent row is formed by standing on the
   * shoulders
   * of the row below. Each row had one less person than the row below it, and the
   * topmost
   ...
   * G   H   I   J
   * 
   * Each person in the pyramid has a weight, and each person supports their own
   * weight
   * plus half the weight of each person directly above them. For example, in the
   * pyramid
   * E supports half the weight supported by B and half the weight supported by C,
   * in
   * addition to their own weight.
   * 
   * Given a human pyramid represented as a list of lists of integers, where each
   * integer
   * represents the weight of a person, write a function that calculates the total
   * weight
   ...
   * 3. The position in the pyramid is given as a pair of integers (row, col),
   * where row
   * is the row index (0-indexed from the top) and col is the column index
   ...
   * 
   * @param pyramid - A list of lists of integers representing the weights of
   *                people in the pyramid.
   * @param row     - The row index of the person whose supported weight is to be
   *                calculated.
   * @param col     - The column index of the person whose supported weight is to
   *                be calculated.
   * @returns float - The total weight supported by the person at the given
   *          position.
   */
  ...
  public double totalWeightSupported(List<List<Double>> pyramid, int row, int col) {
    ...
    double ownWeight = pyramid.get(row).get(col);

    if(col == 0) {
      return ownWeight + 0.5 * totalWeightSupported(pyramid, row - 1, col);
    }

    if (row == col) {
      return ownWeight + 0.5 * totalWeightSupported(pyramid, row - 1, col - 1);
    }

    return ownWeight + 0.5 * totalWeightSupported(pyramid, row - 1, col - 1) + 0.5 * totalWeightSupported(pyramid, row - 1, col);
  }
